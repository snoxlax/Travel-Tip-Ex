...
function getLocCountByUpdates() {
  return storageService.query(DB_KEY).then((locs) => {
    const locCountByUpdates = locs.reduce(
      (map, loc) => {
        ...
        if (loc.updatedAt === loc.createdAt) map.never++;
          ...
          map.today++;
        else if (/^\d+\s+days ago$/.test(time)) map.past++;
        return map;
      },
...
function _createDemoLocs() {
  var locs = [
    ...
    {
      name: 'HaNegev St 13, Ashdod, Israel',
      rate: 4,
      geo: {
        address: 'HaNegev St 13, Ashdod, Israel',
        lat: 31.8026398,
        lng: 34.66410219999999,
        zoom: 8,
      },
    },
    {
      name: 'St 16, Haifa, Israel',
      rate: 3,
      geo: {
        address: 'St 16, Haifa, Israel',
        lat: 32.7783473,
        lng: 35.0156462,
        zoom: 8,
      },
